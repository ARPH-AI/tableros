...
Required infrastructure it's ´docker-compose´ defined.
#TODO: add docker-compose installation steps

Services:
  * postgres-dashboard
    * runs `postgres v11.6`
    * defines ENV VARS for postgres initial setup
      - `POSTGRES_DB`
      - `POSTGRES_USER`
      - `POSTGRES_PASSWORD`
    * persists database data at: `dashboard-psqlserver-volume`
    * exposed at port: `5532`
    * maps host's `./databases/scripts/init/` to postgres init scripts directory `/docker-entrypoint-initdb.d/`
  * cube
    * runs `cubejs/cube:v0.26.44`
    * sources ENV VARS from `./cubejs/.env`
    * exposed at ports:
      + `4040` Cube.js API
      + `3030` Playground web-server
    * configuration it's mapped to host's `./cubejs/`

Generate required environment variables file with `make envfile`.
Run infrastructure with `make start`.

#### Since postgres-dashboard service maps init scripts from host, Dashboard database required roles and schema are already created from them.

> **Feel free to update them when required, just by editing `./databases/scripts/init/` content.**

> **Scripts order is defined by file name.**

Database can be handled using `make` directives defined within `./Makefile`.

Find them all [here](). #TODO: link to repo file url.
